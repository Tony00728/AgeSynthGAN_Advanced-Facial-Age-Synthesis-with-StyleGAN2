...
class AgingLoss(nn.Module):
    ...
    def __get_predicted_age(self, age_pb):
        #print("age_pb:", age_pb.shape)         # torch.Size([4, 101]), ages from 0 to 100
        predict_age_pb = F.softmax(age_pb, dim=1)   # 有警告可能有問題 # Apply softmax along the second dimension (dim=1)
    ...
    def extract_ages(self, x):
        x = F.interpolate(x, size=(224, 224), mode='bilinear', align_corners=True) #改動
